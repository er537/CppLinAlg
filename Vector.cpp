...
//constuctor that creates a vector of length 0
Vector::Vector()
{
  mSize = 0;
  mData = NULL;
...
bool compare(const Vector& v1, const Vector& v2){
	if (v1.mSize == v2.mSize){
		for (int i = 0; i < v1.mSize;i++){
			if (v1.mData[i]!=v2.mData[i]){
				return false;
			}
		}
		return true; //return true if all entries are the same and they are the same size. Otherwise return false.
	}
	return false;
}
